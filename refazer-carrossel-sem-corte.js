...
fs.writeFileSync("public/index-antes-refazer-carrossel-sem-corte.html", html, "utf8");
      ...
      <section class="section reveal" id="carrosselOnze">
        <div class="section-head">
          <div>
            ...
            <p>
              Confira os 11 prêmios especiais. Navegue pelos cards usando os botões, arrastando para o lado ou deslizando no celular.
            </p>
          </div>
              ...
              return `
                <article class="carousel-11-card">
                  ...
                  <div class="carousel-11-content">
                    ...
                    <h3>Prêmio ${number}</h3>
                    <p>
                      Prêmio especial da Copa Solar. A imagem acima aparece inteira, sem cortes.
                    </p>
                  </div>
                </article>
              `;
...
const carouselCss = `
  <style id="carousel-11-style">
    ...
    .carousel-11-wrap {
      ...
      background:
        radial-gradient(circle at 15% 15%, rgba(211, 255, 0, .9), transparent 25%),
        radial-gradient(circle at 88% 20%, rgba(33, 211, 202, .9), transparent 28%),
    ...
    .carousel-11-card:nth-child(3n + 1) {
      background: linear-gradient(135deg, #d6ff00, #ffffff 74%);
    }

    .carousel-11-card:nth-child(3n + 2) {
      background: linear-gradient(135deg, #21d3ca, #ffffff 74%);
    }

    .carousel-11-card:nth-child(3n + 3) {
      background: linear-gradient(135deg, #ff8e8e, #ffffff 74%);
    }
...
// Substitui seção atual inteira
html = html.replace(
  /<section class="section reveal" id="carrosselOnze">[\s\S]*?<\/section>/,
  carouselSection
);

// Substitui CSS antigo do carrossel
html = html.replace(
  /<style id="carousel-11-style">[\s\S]*?<\/style>/,
  carouselCss
);

// Remove CSS antigo extra das imagens, se existir
html = html.replace(
  /<style id="carousel-11-image-style">[\s\S]*?<\/style>/,
  ""
);

// Substitui JS antigo do carrossel
html = html.replace(
  /function setupCarousel11\(\) \{[\s\S]*?\n\}/,
  carouselJs
);

fs.writeFileSync(path, html, "utf8");

console.log("Carrossel refeito sem cortes nas imagens.");